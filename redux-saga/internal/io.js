import delayP from '../dependencies/delay-p';
import * as is from '../dependencies/is';
import { IO, SELF_CANCELLATION } from '../dependencies/symbols';
import { check, createSetContextWarning, identity } from './utils';
import * as effectTypes from './effectTypes';

const TEST_HINT =
  '\n(HINT: if you are getting this errors in tests, consider using createMockTask from @redux-saga/test-utils)';
...
export function take(patternOrChannel = '*', multicastPattern) {
  ...
  if (
    is.multicast(patternOrChannel) &&
    is.notUndef(multicastPattern) &&
    is.pattern(multicastPattern)
  ) {
  ...
  throw new Error(
    `take(patternOrChannel): argument ${patternOrChannel} is not valid channel or a valid pattern`
  );
}
...
const validateFnDescriptor = (effectName, fnDescriptor) => {
  check(
    fnDescriptor,
    is.notUndef,
    `${effectName}: argument fn is undefined or null`
  );
  ...
  if (is.array(fnDescriptor)) {
    ...
    check(
      fn,
      is.notUndef,
      `${effectName}: argument of type [context, fn] has undefined or null \`fn\``
    );
  ...
  if (context && is.string(fn)) {
    check(
      context[fn],
      is.func,
      `${effectName}: context arguments has no such method - "${fn}"`
    );
    return;
  }
...
export function actionChannel(pattern, buffer) {
  if (process.env.NODE_ENV !== 'production') {
    check(
      pattern,
      is.pattern,
      'actionChannel(pattern,...): argument pattern is not valid'
    );
...
export function getContext(prop) {
  if (process.env.NODE_ENV !== 'production') {
    check(
      prop,
      is.string,
      `getContext(prop): argument ${prop} is not a string`
    );
  }
