...
import { SAGA_ACTION, TASK, TASK_CANCEL, TERMINATE } from '../dependencies/symbols'
...
`
/* 
 * action不应该是frozen状态的，因为saga需要给action添加一个不可枚举的SAGA_ACTION属性，
 * 标记action来自saga内部的put产生
 * 如果实在需要使action为frozen状态，应该添加一个中间件而非创建一个frozen action
 */
...
/**
 * @param {Array|Object} shape -> {a: fn1, b: fn2}
 * @param {Function} parentCallback -> (res, isErr) => cb(res, isErr)
 * 类似Promise.all([fn1, fn2]) => ([res1, res2]) => parentCallback([res1, res2])
 */
